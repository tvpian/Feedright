...
# ── Make sure both `apps/` and `packages/` are importable ─────────────────────
_root = Path(__file__).resolve().parent.parent
sys.path.insert(0, str(_root / "apps"))
sys.path.insert(0, str(_root / "packages"))

# ── Import the ORM Base so autogenerate can diff against it ───────────────────
...
# Allow DATABASE_URL env var to override alembic.ini value (useful in prod)
